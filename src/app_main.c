...
int   openConn(const char *streamHost, const char *streamPath)
{
	int n, i;
	while(1) {
...
void  tskconnect(void *pvParameters)
{
	...
	struct sdk_station_config *config=malloc(sizeof(struct sdk_station_config));
	memset(config, 0x00, sizeof(struct sdk_station_config));
...
extern void sdk_os_update_cpu_frequency(int mhz);
